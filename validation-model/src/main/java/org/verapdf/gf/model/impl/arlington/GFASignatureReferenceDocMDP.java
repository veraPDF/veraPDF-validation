...
public class GFASignatureReferenceDocMDP extends GFAObject implements ASignatureReferenceDocMDP {

	public GFASignatureReferenceDocMDP(COSBase baseObject, COSBase parentObject, String keyName) {
		super(baseObject, parentObject, keyName, "ASignatureReferenceDocMDP");
	}
	...
	private List<ADocMDPTransformParameters> getTransformParams1_5() {
		...
		if (object.getType() == COSObjType.COS_DICT) {
			List<ADocMDPTransformParameters> list = new ArrayList<>(1);
			list.add(new GFADocMDPTransformParameters((COSDictionary)object.getDirectBase(), this.baseObject, "TransformParams"));
			return Collections.unmodifiableList(list);
		}
		return Collections.emptyList();
	}
