...
public class GFAFontFile2 extends GFAObject implements AFontFile2 {
	...
	private List<org.verapdf.model.baselayer.Object> getFDecodeParms1_1() {
		...
		if (object.getType() == COSObjType.COS_DICT) {
			List<AFilterLZWDecode> list = new ArrayList<>(1);
			list.add(new GFAFilterLZWDecode((COSDictionary)object.getDirectBase(), this.baseObject, "FDecodeParms"));
			return Collections.unmodifiableList(list);
		}
		return Collections.emptyList();
	}
	...
	private List<AArrayOfCompressionFilterNames> getFFilter() {
		switch (StaticContainers.getFlavour()) {
			...
			case ARLINGTON1_7:
			case ARLINGTON2_0:
				return getFFilter1_1();
			default:
				return Collections.emptyList();
		}
	}

	private List<AArrayOfCompressionFilterNames> getFFilter1_1() {
	...
	public COSObject getparentparentSubtypeValue() {
	...
	@Override
	public String getparentparentSubtypeNameValue() {
		COSObject parentparentSubtype = getparentparentSubtypeValue();
		return getNameValue(parentparentSubtype);
	}
