...
public class GFAAnnotFileAttachment extends GFAObject implements AAnnotFileAttachment {

	public GFAAnnotFileAttachment(COSBase baseObject, COSBase parentObject, String keyName) {
		super(baseObject, parentObject, keyName, "AAnnotFileAttachment");
	}
	...
	private List<AFileSpecification> getFS() {
		switch (StaticContainers.getFlavour()) {
			case ARLINGTON1_3:
			case ARLINGTON1_4:
			case ARLINGTON1_5:
	...
	private List<AFileSpecification> getFS1_3() {
		...
		if (object.getType() == COSObjType.COS_DICT) {
			...
			list.add(new GFAFileSpecification((COSDictionary)object.getDirectBase(), this.baseObject, "FS"));
			return Collections.unmodifiableList(list);
		}
		return Collections.emptyList();
	}
	...
	public Boolean getFSHasTypeDictionary() {
		COSObject FS = getFSValue();
		return getHasTypeDictionary(FS);
	}

	@Override
	public Boolean getFSHasTypeString() {
		COSObject FS = getFSValue();
		return getHasTypeString(FS);
	}
	...
	public COSObject getNameDefaultValue() {
		switch (StaticContainers.getFlavour()) {
			...
			case ARLINGTON1_7:
			case ARLINGTON2_0:
				return COSName.construct("PushPin");
		}
		return null;
	}
