...
import org.verapdf.model.factory.operators.TransparencyGraphicsState;
...
import org.verapdf.model.impl.pd.images.GFPDXForm;
import org.verapdf.model.impl.pd.images.GFPDXImage;
...
public class TransparencyBehaviour {
	...
	private boolean fontCheck(TransparencyGraphicsState graphicState) {
		...
		if ((fontId == null || !StaticContainers.transparencyCheckedSet.contains(fontId)) && font instanceof GFPDType3Font) {
			if (fontId != null) {
				StaticContainers.transparencyCheckedSet.add(fontId);
			}
	...
	private boolean colorSpaceCheck(TransparencyGraphicsState graphicState) {
		if (isFillCheck) {
			...
			String patternID = fillCS.getID();
			if (patternID == null || !StaticContainers.transparencyCheckedSet.contains(patternID)) {
				if (patternID != null) {
					StaticContainers.transparencyCheckedSet.add(patternID);
				}
		...
		if (isStrokeCheck) {
			...
			String patternID = strokeCS.getID();
			if (patternID == null || !StaticContainers.transparencyCheckedSet.contains(patternID)) {
				if (patternID != null) {
					StaticContainers.transparencyCheckedSet.add(patternID);
				}
	...
	private boolean xObjectContainsTransparency(GFPDXObject xobj) {
		String xobjId = xobj.getID();
		if (xobjId == null || !StaticContainers.transparencyCheckedSet.contains(xobjId)) {
			if (xobjId != null) {
				StaticContainers.transparencyCheckedSet.add(xobjId);
			}
